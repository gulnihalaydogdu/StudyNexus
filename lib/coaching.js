...
export function assignPlanToStudent({ teacherId, studentId, planId, year, month, week }) {
    const link = dbGet(
        'SELECT id FROM teacher_student_links WHERE teacher_id = ? AND student_id = ?',
        [teacherId, studentId]
    );
    ...
    dbRun(
        ...
        (teacher_id, student_id, source_plan_id, student_plan_id, target_year, target_month, target_week)
        VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [teacherId, studentId, planId, studentPlanId, year, month, week]
    );
