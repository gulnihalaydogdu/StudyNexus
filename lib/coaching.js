...
export function assignPlanToStudent({ teacherId, studentId, planId, month, week }) {
    const link = dbGet(
        'SELECT id FROM teacher_student_links WHERE teacher_id = ? AND student_id = ?',
        [teacherId, studentId]
    );
    ...
    dbRun(
        ...
        [studentId, month, week]
    );
    dbRun(
        'INSERT INTO calendar_events (user_id, plan_id, target_month, target_week) VALUES (?, ?, ?, ?)',
        [studentId, studentPlanId, month, week]
    );

    dbRun(
        `INSERT INTO plan_assignments
        (teacher_id, student_id, source_plan_id, student_plan_id, target_month, target_week)
        VALUES (?, ?, ?, ?, ?, ?)`,
        [teacherId, studentId, planId, studentPlanId, month, week]
    );
