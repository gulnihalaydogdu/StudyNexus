...
export function changeEmailEmail({ name, confirmUrl, newEmail }) {
    const safeName = escapeHtml(name || 'Kullanıcı');
    const safeEmail = escapeHtml(newEmail || '');
    return {
        subject: 'StudyNexus — E-posta değişikliğini onaylayın ✉️',
        html: emailShell({
            preheader: 'StudyNexus hesabınız için yeni e-posta adresini onaylayın.',
            ...
            bodyHtml: `
                ...
                <p style="margin:0 0 16px;text-align:center;">
                  StudyNexus hesabınız için <strong>${safeEmail}</strong> adresine geçiş talebi aldık.
                  Değişikliği tamamlamak için aşağıdaki butona tıklayın.
                </p>
                <p style="margin:0;text-align:center;font-size:14px;color:${BRAND.muted};">
                  Bu talebi siz yapmadıysanız e-postayı yok sayın; mevcut adresiniz geçerli kalır.
                </p>
            `,
            ctaLabel: 'E-postayı Onayla',
            ctaUrl: confirmUrl,
            footerNote: 'Onaylanana kadar eski e-posta adresiniz kullanılmaya devam eder.'
        })
    };
}
