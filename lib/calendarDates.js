...
const shortMonthTR = (date) => MONTH_NAMES[date.getMonth()].slice(0, 3);

export function formatWeekRangeParts(weekStart, weekEnd) {
    ...
    const startMonth = shortMonthTR(s);
    const endMonth = shortMonthTR(e);
    const sameMonth = s.getMonth() === e.getMonth() && s.getFullYear() === e.getFullYear();
    return { sameMonth, startDay, endDay, startMonth, endMonth };
}

export function formatWeekRangeTR(weekStart, weekEnd) {
    const { sameMonth, startDay, endDay, startMonth, endMonth } = formatWeekRangeParts(weekStart, weekEnd);
    ...
    for (let guard = 0; guard < 6; guard++) {
        ...
        if (overlaps) {
            weeks.push({
                ...
                labelParts: formatWeekRangeParts(weekStart, weekEnd),
                year,
